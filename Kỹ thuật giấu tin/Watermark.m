...
M = zeros(size(Bw_double));
for i = 1:size(Bw_double,1)
    for j = 1:size(Bw_double,2)
        M(i,j) = mean(Bw_double{i,j}(:));
    end
end
...
for i = 1:size(Bw_double,2)*size(Bw_double,1)
    ...
    output_1{a,b} = PixelMovement(output_1{a,b},bit1_idx(i));
...
for i = 1:size(Bw_double,2)*size(Bw_double,1)
    ...
    output_3{a,b} = InversePixelMovement(output_3{a,b},bit1_idx(i));
end
...
psnr_value = psnr(double(img),image_w);
disp(psnr_value);
